  ...
  return (
    <div className="flex flex-1 flex-col items-center gap-1.5">
      ...
      <span
        className={cn(
          "text-[10.5px]",
          variant === "current" ? "font-semibold text-navy-900" : "text-navy-500/80"
        )}
      >
        {label} · {value}
      </span>
...
function AcquisitionRow({
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 text-[13.5px] text-navy-900">{label}</span>
      <span className="text-[11.5px] text-navy-500/80">
        {value === 0 ? "sem movimento" : "no mês"}
      </span>
      <span
        className={cn(
          "w-7 text-right text-lg font-bold tracking-tight",
          value === 0 ? "text-navy-100" : "text-navy-900"
        )}
      >
        {value}
      </span>
  ...
  return (
    ...
    >
      <div className="grid md:grid-cols-[1.05fr_1fr]">
        <section className="flex flex-col gap-4 border-navy-100 p-6 md:border-r">
          <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-navy-500/70">
            Base de clientes ativos
          </p>

          <div className="flex items-end gap-3">
            <span className="text-[54px] font-bold leading-[0.9] tracking-[-0.04em] text-navy-900">
              {currentMonth}
            </span>
          ...
          <div className="flex items-end gap-2.5 border-t border-navy-100/70 pt-3">
        ...
        <section className="flex flex-col gap-3 p-6">
          ...
          {acquisition.leads + acquisition.potential + acquisition.new === 0 ? (
            <div className="mt-auto flex items-center gap-2 rounded-lg bg-navy-100/40 px-3 py-2.5 text-[12.5px] text-navy-500">
