...
function MetricRow({
  ...
  const pastBarColor = theme === "dark" ? "#3a4a6b" : "#dbe2ee";
  ...
  return (
    ...
    >
      <p className="pb-1 text-xs font-semibold text-navy-500 dark:text-navy-100/70">{label}</p>
      {COLUMNS.map((column) => {
        ...
        return (
          <div key={column.key} className="flex flex-col gap-1.5">
            <div className="flex items-baseline gap-2">
              <span className="text-[26px] font-bold leading-none tracking-[-0.035em] text-navy-900 dark:text-navy-100">
                {value}
              </span>
              <DeltaBadge value={deltas[column.key]} />
            </div>
            <span className="h-[5px] overflow-hidden rounded-full bg-navy-100/70 dark:bg-navy-800/60">
              <span
                className={cn(
                ...
                style={{
                  ...
                  ...(!isCurrent && !isForecast ? { backgroundColor: pastBarColor } : null),
                }}
              />
            </span>
          </div>
...
export function PeriodCards({
  ...
  return (
    ...
    >
      <div className="grid grid-cols-[120px_repeat(3,1fr)] gap-3.5 border-y border-navy-100 bg-navy-100/25 px-5 py-2.5 text-[10px] font-semibold uppercase tracking-[0.09em] text-navy-500/70 dark:border-navy-700 dark:bg-navy-800/40 dark:text-navy-100/50">
        <span />
        {COLUMNS.map((column) => (
          <span
            key={column.key}
      ...
      <footer className="flex flex-wrap items-center gap-2 border-t border-navy-100 bg-navy-100/20 px-5 py-3 text-xs text-navy-500 dark:border-navy-700 dark:bg-navy-800/40 dark:text-navy-100/70">
        <span>
          Containers por booking:{" "}
          <b className="text-navy-900 dark:text-navy-100">
