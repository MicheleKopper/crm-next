...
export async function getCustomerTotal(): Promise<CustomerTotalSummary> {
...
export async function getShipmentsSummary(): Promise<PeriodSummary> {
...
export async function getContainerSummary(): Promise<PeriodSummary> {
  const rows = await prisma.$queryRaw<
    { lastMonth: number; currentMonth: number; nextMonth: number }[]
  >`
    SELECT
...
export async function getAnnualShipments(): Promise<AnnualShipmentPoint[]> {
  return prisma.$queryRaw<AnnualShipmentPoint[]>`
    ...
    LEFT JOIN shipments s
      ON DATE_TRUNC('month', COALESCE(s.atd, s.etd)) = m.month
...
export async function getShipmentTypeBreakdown(): Promise<ShipmentTypePoint[]> {
  return prisma.$queryRaw<ShipmentTypePoint[]>`
    ...
    shipments_agg AS (
      ...
      FROM shipments s
      WHERE COALESCE(s.atd, s.etd) IS NOT NULL
        AND COALESCE(s.atd, s.etd) >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '12 months')
        AND s.status <> 'Cancelled'
...
export async function getStatusShipments(): Promise<StatusShipmentRow[]> {
